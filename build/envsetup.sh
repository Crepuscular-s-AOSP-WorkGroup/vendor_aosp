...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/aosp-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop org.aosp.device | grep -q "$CUSTOM_BUILD"); then
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop org.aosp.device | grep -q "$CUSTOM_BUILD");
    then
...
function pixelgerrit() {
    ...
    local user=`git config --get review.gerrit.aosp.org.username`
...
function dopush()
{
    ...
    if (adb shell getprop org.aosp.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
